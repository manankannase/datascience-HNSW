...
logfile = 'experiment_' + datetime.datetime.now().strftime("%Y%m%d_%H%M%S") + '.log'
logging.basicConfig(
    filename=logfile,
    level=logging.INFO,
...
def run_experiments(data, queries, gt_idx, k_values, cluster_options, top_cluster_options):
        ...
        for n_clusters in cluster_options:
            ...
            for top_c in top_cluster_options:
                ...
                start_query = time.time()
                 # retrieved = []
                retrieved = Parallel(n_jobs=12)(
                   delayed(clustered.query)(q, k=k, top_clusters=top_c) for q in tqdm(queries, desc=f"Querying k={k}")
                )
    ...
    # np.savetxt("experiment_results.csv", results, fmt="%.6f", delimiter=",", header="k,n_clusters,top_clusters,recall,avg_distance")
    logging.info("Saved results to experiment_results.csv")
...
if __name__ == "__main__":
    ...
    logging.info("Available CPUs: %d", multiprocessing.cpu_count())
    ...
    cluster_options = [128, 256, 512, 1024]
